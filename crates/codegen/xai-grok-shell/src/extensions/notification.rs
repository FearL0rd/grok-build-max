...
/// Debug-only details for one durable memory-v2 observation.
///
/// This is populated only when `memory_v2.capture_status_enabled` is true.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct MemoryCaptureDebugEntry {
    /// Concise observation generated by the extraction model.
    pub statement: String,
    /// Optional supporting detail generated for the observation.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub body: Option<String>,
    /// Absolute path to the committed observation file.
    pub path: String,
}
...
pub(crate) fn project_result_usage(result: &mut serde_json::Value, usage: &PromptUsage) {
    ...
    if hide_costs {
        if cost_is_partial {
            result.insert("cost_is_partial".into(), true.into());
        }
        ...
        result.insert("total_cost_usd_ticks".into(), serde_json::json!(ticks));
    }
    if !usage.model_usage.is_empty() {
        ...
        for (name, m) in &usage.model_usage {
            ...
            {
                entry.insert("costUSD".into(), serde_json::json!(ticks_to_usd(ticks)));
            }
            model_usage.insert(name.clone(), entry);
        }
        result.insert("modelUsage".into(), model_usage.into());
    }
}
...
pub enum SessionUpdate {
    ...
    /// Lifecycle update for durable memory-v2 observation capture.
    MemoryCaptureActivity {
        /// queued, running, completed, no_op, retry, or failed.
        activity: String,
        /// First successful durable root turn covered by this job.
        from_turn: u32,
        /// Last successful durable root turn covered by this job.
        through_turn: u32,
        /// Lease attempt number; zero while merely queued.
        attempt: u32,
        /// Failure class/details only.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        detail: Option<String>,
        /// Generated observations, present only for debug-enabled successful captures.
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
        memories: Vec<MemoryCaptureDebugEntry>,
    },
    /// Memory dream consolidation is queued.
    MemoryDreamQueued,
    /// Memory dream consolidation acquired its canonical-editor lease.
    MemoryDreamStarted {
        /// Number of observations in the fixed snapshot.
    ...
    MemoryFiles {
        ...
        /// Whether memory is active for this session. Older shells omit it; default `true` keeps the pre-flag behavior.
        #[serde(default = "default_true")]
        enabled: bool,
        /// Why memory is off. Only meaningful when `enabled` is false.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        disabled_reason: Option<MemoryDisabledReason>,
        /// Notes are written automatically after turns. Drives the modal's empty-state advice.
        #[serde(default = "default_true")]
...
/// Why memory is off for the session, so the `/memory` modal can say whether the user can turn it back on.
...
pub enum MemoryDisabledReason {
    ...
    NotConfigured,
    /// A reason this pager predates. Treated as not re-enableable.
    #[serde(other)]
    Unknown,
}
...
mod tests {
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        ...
        let [d0, d1] = parsed.attempt_details.as_slice() else {
            panic!("expected two attempt details: {:?}", parsed.attempt_details);
        };
        assert_eq!(d0.outcome, "degenerate");
        assert_eq!(d0.summary.as_deref(), Some("Now I will do X, Y, Z."));
        assert_eq!(d1.outcome, "success");
        assert_eq!(d1.summary, None);
    }
    ...
    fn subagent_progress_serializes_snake_case_tag() {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!("subagent_progress"))
        );
        // Fields serialize as snake_case (Rust field names): enum `rename_all` only applies to the tag, not struct fields
        assert_eq!(json.get("subagent_id"), Some(&serde_json::json!("sub-1")));
        assert_eq!(
            json.get("parent_session_id"),
            Some(&serde_json::json!("parent-1"))
        );
        assert_eq!(
            json.get("child_session_id"),
            Some(&serde_json::json!("child-1"))
        );
        assert_eq!(json.get("duration_ms"), Some(&serde_json::json!(5000)));
        assert_eq!(json.get("turn_count"), Some(&serde_json::json!(3)));
        assert_eq!(json.get("tool_call_count"), Some(&serde_json::json!(12)));
        assert_eq!(json.get("tokens_used"), Some(&serde_json::json!(45_000)));
        assert_eq!(
            json.get("context_window_tokens"),
            Some(&serde_json::json!(256_000))
        );
        assert_eq!(json.get("context_usage_pct"), Some(&serde_json::json!(35)));
        assert_eq!(
            json.get("tools_used"),
            Some(&serde_json::json!(["bash", "grep"]))
        );
        assert_eq!(json.get("error_count"), Some(&serde_json::json!(1)));
    }
    ...
    fn subagent_progress_orders_after_spawned_before_finished() {
        ...
        assert_eq!(
            progress.get("sessionUpdate"),
            Some(&serde_json::json!("subagent_progress"))
        );
        assert_eq!(
            finished.get("sessionUpdate"),
            Some(&serde_json::json!("subagent_finished"))
        );
    }
    ...
    fn subagent_spawned_agent_address_serializes_as_typed_camel_case() {
        ...
        let json = serde_json::to_value(&update).unwrap();
        assert_eq!(
            json.get("agentAddress"),
            Some(&serde_json::json!("opaque-address"))
        );
        ...
        assert_eq!(
            live.pointer("/update/agentAddress"),
            Some(&serde_json::json!("opaque-address"))
        );
    }
    ...
    fn subagent_finished_with_tokens_used_roundtrips() {
        ...
        let json = serde_json::to_value(&update).unwrap();
        assert_eq!(json.get("tokens_used"), Some(&serde_json::json!(75_000)));
    }
    ...
    #[test]
    fn memory_capture_activity_without_debug_entries_roundtrips() {
        let update = SessionUpdate::MemoryCaptureActivity {
    ...
    #[test]
    fn memory_capture_activity_with_debug_entries_roundtrips() {
        let update = SessionUpdate::MemoryCaptureActivity {
        ...
        let json = serde_json::to_string(&update).unwrap();
        assert!(json.contains("Run focused tests before committing."));
        assert_eq!(
            serde_json::from_str::<SessionUpdate>(&json).unwrap(),
            update
        );
    ...
    fn memory_files_variant_roundtrips_through_json() {
        ...
        assert_eq!(
            json.get("sessionUpdate"),
            Some(&serde_json::json!("memory_files"))
        );
        let Some(files) = json.get("files").and_then(|v| v.as_array()) else {
            panic!("expected files array: {json}");
        };
        assert_eq!(files.len(), 2);
        // Populated timestamp serializes as a plain u64
        assert_eq!(
            json.pointer("/files/0/modified_epoch_secs"),
            Some(&serde_json::json!(1_700_000_000_u64))
        );
        // None is omitted entirely
        assert!(
            json.pointer("/files/1")
                .and_then(|f| f.get("modified_epoch_secs"))
                .is_none()
        );
    }
    ...
    #[test]
    fn memory_files_without_enabled_defaults_to_enabled() {
        let parsed: SessionUpdate =
            serde_json::from_str(r#"{"sessionUpdate":"memory_files","files":[]}"#).unwrap();
        assert_eq!(
            parsed,
        ...
        // A reason from a newer shell must not reject the whole update.
        let parsed: SessionUpdate = serde_json::from_str(
            r#"{"sessionUpdate":"memory_files","files":[],"enabled":false,"disabled_reason":"future_reason"}"#,
        )
        .unwrap();
        assert!(matches!(
            parsed,
            SessionUpdate::MemoryFiles {
                disabled_reason: Some(MemoryDisabledReason::Unknown),
                ..
            }
        ));
    }
    ...
    fn tool_call_delta_chunk_first_event_serializes_with_id_and_name() {
        ...
        assert_eq!(
            json.get("sessionUpdate"),
            Some(&serde_json::json!("tool_call_delta_chunk"))
        );
        assert_eq!(
            json.get("tool_call_id"),
            Some(&serde_json::json!("call_abc"))
        );
        assert_eq!(json.get("tool_index"), Some(&serde_json::json!(0)));
        assert_eq!(json.get("name"), Some(&serde_json::json!("search_replace")));
    ...
    fn tool_call_delta_chunk_subsequent_event_carries_only_arguments_delta() {
        ...
        assert_eq!(json.get("tool_index"), Some(&serde_json::json!(0)));
        assert_eq!(
            json.get("arguments_delta"),
            Some(&serde_json::json!("{\"file\":\"src/"))
        );
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(
            json.get("sessionUpdate"),
            Some(&serde_json::json!("goal_updated"))
        );
        assert_eq!(json.get("goal_id"), Some(&serde_json::json!("g-1")));
        assert_eq!(json.get("status"), Some(&serde_json::json!("active")));
        assert_eq!(json.get("phase"), Some(&serde_json::json!("executing")));
        assert_eq!(json.get("token_budget"), Some(&serde_json::json!(100_000)));
        assert_eq!(json.get("tokens_used"), Some(&serde_json::json!(25_000)));
        assert_eq!(json.get("total_deliverables"), Some(&serde_json::json!(3)));
        assert_eq!(
            json.get("completed_deliverables"),
            Some(&serde_json::json!(1))
        );
        assert_eq!(
            json.get("current_deliverable_idx"),
            Some(&serde_json::json!(2))
        );
        assert_eq!(json.get("total_worker_rounds"), Some(&serde_json::json!(4)));
        assert_eq!(json.get("total_verify_rounds"), Some(&serde_json::json!(2)));
        assert_eq!(
            json.get("live_subagent_tokens"),
        ...
        assert_eq!(json.get("live_context_pct"), Some(&serde_json::json!(35)));
        assert_eq!(
            json.get("last_event"),
            Some(&serde_json::json!("worker_completed"))
        );
        assert_eq!(
            json.get("classifier_runs_attempted"),
            Some(&serde_json::json!(2))
        );
        assert_eq!(json.get("classifier_max_runs"), Some(&serde_json::json!(3)));
        assert_eq!(
            json.get("last_classifier_verdict"),
            Some(&serde_json::json!("not_achieved"))
        );
        assert_eq!(
            json.get("last_classifier_details_path"),
            Some(&serde_json::json!("/tmp/details.md"))
        );
        assert_eq!(
            json.get("verifying_completion"),
            Some(&serde_json::json!(true))
        );
        assert_eq!(json.get("planning"), Some(&serde_json::json!(true)));
    }
    ...
    #[test]
    fn plan_keep_signals_serialize_snake_case_tags() {
        let kept = SessionUpdate::PlanKept {
            plan_uri: "file:///tmp/p.plan.md".into(),
            content: "# Build\n".into(),
        };
        let json = serde_json::to_value(&kept).unwrap();
        assert_eq!(
            json.get("sessionUpdate"),
            Some(&serde_json::json!("plan_kept"))
        );
        assert_eq!(
            json.get("plan_uri"),
            Some(&serde_json::json!("file:///tmp/p.plan.md"))
        );
        assert_eq!(json.get("content"), Some(&serde_json::json!("# Build\n")));
        let parsed: SessionUpdate = serde_json::from_value(json).unwrap();
        assert_eq!(kept, parsed);

        let cleared = serde_json::to_value(&SessionUpdate::PlanCleared).unwrap();
        assert_eq!(
            cleared.get("sessionUpdate"),
            Some(&serde_json::json!("plan_cleared"))
        );

        let executing = serde_json::to_value(&SessionUpdate::PlanExecuting).unwrap();
        assert_eq!(
            executing.get("sessionUpdate"),
            Some(&serde_json::json!("plan_executing"))
        );
        assert!(executing.get("turn_id").is_none());
    }
    ...
    fn turn_completed_serializes_snake_case_tag_and_fields() {
        ...
        assert_eq!(
            json.get("sessionUpdate"),
            Some(&serde_json::json!("turn_completed"))
        );
        assert_eq!(json.get("prompt_id"), Some(&serde_json::json!("p-1")));
        assert_eq!(
            json.get("stop_reason"),
            Some(&serde_json::json!("end_turn"))
        );
        assert_eq!(json.get("agent_result"), Some(&serde_json::json!("done")));
        assert_eq!(
            json.get("error_kind"),
            Some(&serde_json::json!("max_tokens_truncation"))
        );
    }
    ...
    fn turn_completed_elapsed_ms_some_roundtrips() {
        ...
        let json = serde_json::to_value(&update).unwrap();
        assert_eq!(json.get("elapsed_ms"), Some(&serde_json::json!(1234)));
        let parsed: SessionUpdate = serde_json::from_value(json).unwrap();
        assert_eq!(update, parsed);
    }
    ...
    fn project_result_hides_costs_when_partial_or_incomplete() {
        ...
        assert_eq!(
            result.get("usage_is_incomplete"),
            Some(&serde_json::json!(true))
        );
        assert!(result.get("total_cost_usd").is_none());
        assert!(result.get("total_cost_usd_ticks").is_none());
    ...
    fn cost_missing_calls_not_on_acp_wire() {
        ...
        assert_eq!(v.get("costIsPartial"), Some(&serde_json::json!(true)));
    }
    ...
    fn project_result_token_identity_uncached_plus_cache_plus_output() {
        ...
        let mut result = serde_json::json!({});
        project_result_usage(&mut result, &usage);
        let Some(usage_obj) = result.get("usage") else {
            panic!("expected usage object: {result}");
        };
        let Some(uncached) = usage_obj.get("input_tokens").and_then(|v| v.as_u64()) else {
            panic!("expected usage.input_tokens: {result}");
        };
        let Some(cache) = usage_obj
            .get("cache_read_input_tokens")
            .and_then(|v| v.as_u64())
        else {
            panic!("expected usage.cache_read_input_tokens: {result}");
        };
        let Some(output) = usage_obj.get("output_tokens").and_then(|v| v.as_u64()) else {
            panic!("expected usage.output_tokens: {result}");
        };
        let Some(total) = usage_obj.get("total_tokens").and_then(|v| v.as_u64()) else {
            panic!("expected usage.total_tokens: {result}");
        };
        ...
        assert_eq!(acp.get("inputTokens"), Some(&serde_json::json!(100)));
        assert_eq!(acp.get("cachedReadTokens"), Some(&serde_json::json!(40)));
        assert_ne!(
            acp.get("inputTokens"),
            result.pointer("/usage/input_tokens")
        );
        assert_eq!(
            result.pointer("/modelUsage/m/inputTokens"),
            Some(&serde_json::json!(60))
        );
        assert_eq!(
            result.pointer("/modelUsage/m/cacheReadInputTokens"),
            Some(&serde_json::json!(40))
        );
        assert_eq!(result.get("total_cost_usd"), Some(&serde_json::json!(0.2)));
        // Exact ticks accompany the float for tick-exact reconciliation.
        assert_eq!(
            result.get("total_cost_usd_ticks"),
            Some(&serde_json::json!(2_000_000_000_i64))
        );
    }
